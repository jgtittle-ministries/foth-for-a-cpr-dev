**FELLOWSHIP OF THE HEART**

*Going Out series*

Week 4

**The Daily Tells**

*Mission close to home opens — the body looks at what is actually happening in the past three weeks of witness practice*

![The daily rhythm, close to home.](images/wk04-daily-tells.jpg)

**COMPANION LESSON PLAN**

*Pilot edition — Covenant Christian Academy of Warrenton*

*Based on the Intentional Journey of the Heart (IJH), Volumes 1–6*

*John G. Tittle • Curriculum draft v1, May 2026*

**Quick Reference Card**

*Print this page on cardstock. Two copies in the room. Wk 4 opens the mission-close-to-home block. The cohort has been doing weekly outside-cohort Tells for three weeks now — tonight’s work is to look honestly at what is actually happening, surface the patterns, and refine the practice for the next two weeks.*

## WEEK 4 — THE DAILY TELLS (90 minutes)

**Aim.** Surface what the past three weeks of outside-cohort Tells have actually produced. Move from one-sentence cohort-circle reports (Wk 2) to substantive examination of patterns: what kinds of receivers; what kinds of moments; what kinds of cost; what kinds of fruit. The body looks at the body’s daily work and weighs what it sees. Refine the Tell practice for Wks 5–6 with specific differentiation by context.

**Anchor scripture.** John 13:35 (‘By this all people will know you are my disciples, if you have love for one another’); 1 Peter 3:15 (‘always being prepared to make a defense… yet do it with gentleness and respect’); Colossians 4:5–6 (‘Walk in wisdom toward outsiders, making the best use of the time. Let your speech always be gracious, seasoned with salt’).

**Connect focus.** Mission close to home. The daily relational world; the immediate sent-context; people each member already knows.

**Mode.** Whole-room. The cohort’s daily-Tells data is shared across cohort lines tonight; junior teens, senior teens, and parents all hear each other’s actual witness data. The cross-cohort visibility is the pedagogical centre.

**Center.** Brief teaching frame (15 min). Then a structured cohort survey: each member contributes ONE specific Tell from the past three weeks (not the most polished; the most honest), with brief data on receiver type, context, and what surfaced. Lead Companion captures patterns on flip chart in real time. Cohort weighs the patterns. Refinement for Wks 5–6 by domain (household, vocational, third-place).

**Between-session practice.** The weekly outside-cohort Tell continues, with a specific differentiation: this week’s Tell is in the HOUSEHOLD context (family-of-origin, present household, immediate domestic relationships). Daily noticing question shifts: ‘Where in my house am I a witness already, and where am I avoiding being one?’ Standing pair check-in this week: ‘What is the household witness asking of you specifically?’

**IJH source.** Vol 1 FL.XXI Household Formation Law (the Shema’s formation mechanism); Vol 2 Exp 9 (Community as Amplifier extended to outward witness); the ordinary-life witness reading of John 13:35 and 1 Peter 3:15.

## WATCH FOR (Week 4 specific risks — read twice)

*Tonight’s risks center on the comparison dynamics that surface when the cohort sees each other’s actual witness data side by side. Some members will feel exposed; some will compare themselves favorably or unfavorably; some will resist the cohort-level pattern surfacing.*

**Comparison and shame. A junior teen hears a parent’s vocational Tell and feels their school-friend Tell is small. A parent hears another parent’s costly family-of-origin Tell and feels their workplace Tell is shallow. The Lead Companion frames at the start: scale of context is not scale of faithfulness. The daily Tell to a sibling or classmate carries the same weight as a vocational Tell to a colleague.**

**Performative reframing. A member whose Tell did not happen reports it as if it did, or describes a non-Tell encounter as a Tell. The cohort’s honesty discipline has been built across Going Deeper and the first weeks of Going Out. The Lead Companion gently redirects when needed: ‘Honest data is the work. The cohort can hold a non-Tell.’**

**The cohort that has lapsed on the practice. If a substantial portion of the cohort has not done the weekly Tell three weeks running, tonight’s data will be thin. Receive this honestly; Going Out re-engages. The team’s discipline is to refuse to manufacture data that did not happen.**

**The Tell that produced unintended consequence. A member’s Tell three weeks ago has now produced a follow-on conversation, a relational shift, an unexpected opening, or a closing of a door. Receive cleanly; pastoral 1:1 within the week to walk what is unfolding.**

**The Tell that revealed the receiver’s active crisis. A friend mentioned in a Tell is now in active mental-health, relational, or safety crisis. The member is carrying this without being equipped to. Refer; pastoral 1:1; clinical referrals as appropriate. Mandatory-reporting law applies as relevant for teen receivers.**

**Cross-cohort family-Tell exposure. A teen’s Tell about their parent (in a different cohort) surfaces. A parent’s Tell about their teen surfaces. The architecture protects: initials only; the cross-cohort person does not enter the discussion in identifying detail; cross-cohort Companions hold the dynamic privately.**

**Theological flattening. A member frames every Tell as evangelism-toward-conversion (the Wk 2 distinction collapses). Re-frame: ‘We are surfacing the body’s daily witness, which includes evangelism but is broader. Tonight is the data, including non-evangelistic Tells.’**

**Politicized Tell content. A member describes a Tell that involved political content or culturally-charged conversation (Christian witness intersecting with current political debates). The architecture holds without adjudicating the political content; the cohort’s discipline is the witness posture, not the political position.**

## CRISIS CONTINGENCIES (Week 4)

*Crisis risk is moderate. The Tell-data review surfaces real-life material that occasionally crosses thresholds.*

**If a teen’s Tell-receiver is in active self-harm or crisis. Section 6 protocol immediately. Mandatory-reporting law applies where relevant. The teen Companion stays close to the teen for the rest of the session.**

**If a Tell revealed abuse or exploitation by an adult known to the cohort member. Mandatory-reporting law applies (Virginia CPS for minor victims; Virginia APS for vulnerable-adult victims). Two-adult rule for any reporting conversation. Pastoral 1:1 within 24 hours.**

**If a member’s Tell pattern reveals burnout in their witness practice (over-extended; exhausted; performing). Receive cleanly; sabbath-rhythm conversation in pastoral 1:1; the Tell practice itself may need to be paused for a week if the member is burning out.**

**If a Tell revealed marital or family rupture (the receiver was the member’s spouse and the Tell exposed long-standing material). Receive briefly; the cohort space is not the venue for marital adjudication; pastoral 1:1 with both partners if appropriate.**

**If the cohort’s data review reveals significant cross-cohort family material (a teen’s Tell of a parent, then the parent’s defensive response in their own contribution). Lead Companion holds the architecture; no adjudication in the cohort space; cross-cohort pastoral conversation outside the session.**

**Default. Section 6 of the Going Deeper Handbook v1.1 (carried forward).**

**Session at a Glance**

**Why this session, this week**

Going Out’s body-sent block (Wks 1–3) framed the architecture of being sent and produced each member’s working sent-context sentence. The mission-close-to-home block (Wks 4–6) makes the architecture concrete in the daily relational world. Wk 4 is the data look — what has the cohort actually been doing for the past three weeks of weekly outside-cohort Tells, and what does the data reveal about who the cohort is becoming as a sent body.

This is corporate-soul-clearing work in a particular form. Vol 2 proposes (in the open unknowns) a Community Soul Clearing Law — the corporate analog of inner healing. The cohort’s data review tonight is one shape of that work: the body looks honestly at the body’s daily life, names what it sees, weighs what is happening, and refines what is asked of itself going forward.

The cross-cohort visibility tonight is intentional. Wks 5 and 6 will split the cohort to engage household and vocational witness with developmentally-appropriate texture. But Wk 4 needs the body to see itself as one body — junior teens hearing what parents’ Tells look like, parents hearing what teens’ Tells look like, the senior cohort hearing both. The diversity of contexts is itself formation; the senior teen learns from the parent’s family-of-origin Tell that this kind of witness is also legitimate; the parent learns from the junior teen’s playground Tell that small daily witness counts.

The architectural discipline tonight is honest data over polished narrative. The Lead Companion captures patterns on the flip chart in real time — not interpretations, observations. ‘Receivers: 7 colleagues, 4 family members, 3 classmates, 2 neighbours, 1 service provider.’ ‘Tells that produced response: 4. Tells that produced silence: 6. Tells the member did not understand the receiver’s reaction: 5.’ The data is what it is; the cohort weighs what it means together.

**Dependencies**

## From prior sessions and prior practice

**•** The past three weeks of outside-cohort Tells. Every member should arrive having done at least one Tell per week; some will have more, some will have lapsed. The data is the data.

**•** Going Out Wk 1–3 architecture. The four-block Going Out; the body-sent posture; each member’s working sent-context sentence.

**•** Going Deeper Wk 9 (gift and shadow). The gift each member named is producing characteristic kinds of Tells; the shadow is producing characteristic risks.

**•** All Rhythm Card practices, especially the morning noticing question and weekly outside-cohort Tell.

**•** Going Deeper Wk 11 (cohort assessment). The cohort’s honest level is what the witness emerges from; Going Out builds where the cohort actually is.

**Pre-Work for the Formation Companion Team (this week)**

**Personal pre-work**

Each Companion does three things this week.

First — do this week’s Tell yourself, paying particular attention. Note the receiver type; the context; what surfaced. Bring the data to the team meeting. The team’s witness practice has to be honest before the team can ask the cohort’s to be.

Second — review your cohort’s past three weeks of Tell reports (whatever you have heard in pair conversations and pastoral 1:1s). Where are the patterns within your cohort? What kinds of receivers? What kinds of cost? What is missing — what kinds of Tells are NOT happening that you might expect to see?

Third — read John 13:35 and 1 Peter 3:15 together. Both passages describe a witness that is partly relational presence and partly responsive defense. Hold both; the cohort’s Tell data tonight will likely span both modes.

**Team pre-work**

Forty-eight hours before Wk 4, the Companion team meets for sixty minutes.

**1.** Each Companion shares their own week’s Tell. The data the team brings shapes the data the team can hold tonight. (10 min)

**2.** Cohort review by Companion. Each Cohort Companion reports what they have heard in their cohort’s past three weeks. Patterns? Gaps? Concerning Tells? Cross-cohort dynamics? (20 min)

**3.** Walk the run sheet. Block 5 (cohort survey) is the architectural centre tonight; team rehearses how to capture data without interpreting prematurely. (15 min)

**4.** Walk the three scriptures briefly. Discuss how the relational-presence and responsive-defense modes both count as witness. (5 min)

**5.** Special cases: cross-cohort family material expected; teens with Tell-receivers in active crisis; members who have lapsed on the practice. (5 min)

**6.** Crisis contingencies and pastoral / clinical backup confirmed. (3 min)

**7.** Pray for each cohort member by name; pray specifically for honest contributions tonight that surface the body’s actual witness life. (2 min)

**Logistics pre-work**

**•** Print the Tell Survey Worksheet (H4.1) — one per participant. Used during Block 5.

**•** Print the Three-Mode Witness Reference (H4.2) — one per participant. Walks relational presence, responsive defense, and intentional initiative as three modes of witness.

**•** Print the Domain Differentiation Card (H4.3) — one per participant. Maps household, vocational, and third-place witness for Wks 5–6.

**•** Print the Between-Session card (H4.4) — one per participant.

**•** Large flip chart for the shared circle, with three column headers pre-drawn: RECEIVER TYPE / CONTEXT / WHAT SURFACED. Lead Companion captures contributions in real time.

**•** Confirm pastoral / clinical backup.

**•** Confirm room layout: ONE large circle of 20–32 chairs; flip chart visible to whole circle.

**Materials and Setup**

**Materials checklist**

**•** Chairs in ONE large circle.


**•** Personal Heart Journals.

**•** Whiteboard with John 13:35 and the three modes pre-drawn.

**•** Large flip chart with three columns: receiver type / context / what surfaced.

**•** Handouts H4.1, H4.2, H4.3, H4.4 stacked at each chair.

**•** Tissues throughout.

**•** Wall clock or visible timer.

**•** Crisis Quick-Reference Card in every Companion’s pocket.

**•** Pastoral / clinical backup confirmed.

**Detailed 90-Minute Run Sheet**

| **Time** | **Block** | **Mode** | **Lead** | **Notes** |
| --- | --- | --- | --- | --- |
| 6:45–7:00 | Arrival | Single circle | Co-Companion | Door. |
| 7:00–7:11 | Block 1: Open and 60-second settling | Shared circle | Lead Companion | Aaronic. Frame: ‘Tonight we look at our daily witness honestly.’ |
| 7:11–7:23 | Block 2: Three-mode teaching frame | Shared circle | Lead Companion | Read John 13:35, 1 Peter 3:15, Col 4:5–6. Three modes of witness. |
| 7:23–7:30 | Block 3: Worksheet preparation | Shared circle | Cohort Companions float | 7 min silent: each member fills H4.1 with ONE specific Tell. |
| 7:30–8:08 | Block 4: Cohort survey around the circle | Shared circle | Lead Companion | Each member 90 sec; Lead captures on flip chart in real time. |
| 8:08–8:16 | Block 5: Pattern naming + cohort weighing | Shared circle | Lead Companion | Lead names patterns from the flip chart; cohort weighs. |
| 8:16–8:21 | Block 6: Wks 5–6 differentiation introduced | Shared circle | Lead Companion | Household (Wk 5) / vocational (Wk 6); H4.3 walked briefly. |
| 8:21–8:30 | Block 7: Feedback round and closing container | Shared circle | Lead Companion | Aaronic. This week’s Tell is household-domain. Send. |

**Block-by-Block: Scripts and Notes**

**Block 1 — Open and 60-Second Settling (7:00–7:11, 11 min)**
## Script

*“Welcome. Settle.”*

*“The Lord bless you and keep you; the Lord make His face shine on you and be gracious to you; the Lord turn His face toward you and give you peace.”*

*(60-second silent settling. ‘Holy Spirit, you have been at work in our daily lives. Show us tonight what you have been doing through us, and through us not yet.’)*

*“Tonight is Wk 4. Mission close to home opens. The cohort has been doing weekly outside-cohort Tells for three weeks; tonight we look at the data honestly. Not to grade ourselves. To see who we are becoming as a sent body in our daily lives.”*

*“Two disciplines tonight. ONE — honest data. If you forgot the Tell three weeks running, that is data; we hold it. If your Tell produced silence, that is data; we hold it. We refuse to manufacture data that did not happen. TWO — no comparison. Scale of context is not scale of faithfulness. The classroom Tell carries the same weight as the boardroom Tell.”*

*“Container reminder: same as it has been. What is named here stays in this room.”*

*“And before we move on: one true sentence. One true thing about your own week with God, however small. ‘Nothing came’ is a true sentence. We receive; we do not fix.”*

*(Around the circle, brief, pass anytime. This beat runs every week of the year — the smallest rung of costly telling, its occasion scheduled so the muscle always has one.)*

**Block 2 — Three-Mode Teaching Frame (7:11–7:23, 12 min)**
## Script (the read)

*“Three passages frame what we are looking at tonight. Listen.”*

*By this all people will know you are my disciples, if you have love for one another.*

*— John 13:35 (ESV)*

*But in your hearts honor Christ the Lord as holy, always being prepared to make a defense to anyone who asks you for a reason for the hope that is in you; yet do it with gentleness and respect.*

*— 1 Peter 3:15 (ESV)*

*Walk in wisdom toward outsiders, making the best use of the time. Let your speech always be gracious, seasoned with salt, so that you may know how you ought to answer each person.*

*— Colossians 4:5–6 (ESV)*

**Teaching points**

**•** Three modes of witness emerge across these passages. RELATIONAL PRESENCE — John 13:35. The body’s love for one another that becomes visible to the world. The witness is who you are with people, not what you say. RESPONSIVE DEFENSE — 1 Peter 3:15. Someone asks; you answer with gentleness and respect. The witness is reactive; the receiver initiates. INTENTIONAL INITIATIVE — Colossians 4:5–6. The body walks in wisdom toward outsiders, making the best use of the time. The witness is proactive; the body moves toward the world.

**•** All three modes are real witness. Most of our daily Tells fall into one of these three categories. A common pattern: the cohort defaults to one mode without realizing the other two are also legitimate. The discipline tonight is seeing what mode each Tell uses, and noticing whether our cohort is using all three or just one.

**•** All three modes have characteristic risks. Relational presence can drift into withholding (the witness becomes invisible because nothing is ever said). Responsive defense can drift into reactivity (the witness only happens when triggered). Intentional initiative can drift into pressing (the witness becomes coercive).

## The three modes

**RELATIONAL PRESENCE. The body’s love made visible. ‘They will know’ by being WITH us — our love for one another, our integrity in ordinary life, our presence under suffering. The witness is woven into how we live. Risk: withholding (the witness becomes invisible).**

**RESPONSIVE DEFENSE. Someone asks; we answer. The witness is reactive but specific — the gentleness and respect matter; the answer matters; the timing is set by the receiver, not the witness. Risk: reactivity (the witness only happens when triggered).**

**INTENTIONAL INITIATIVE. The body walks toward outsiders — specifically, gracefully, with seasoned speech. The witness is proactive, with the wisdom to know how to answer each person. Risk: pressing (the witness becomes coercive).**

*“Tonight’s data look will reveal which modes our cohort uses, which we underuse, and what the patterns mean. We are not grading; we are seeing.”*

**Block 3 — Worksheet Preparation (7:23–7:30, 7 min)**
## Script

*“Seven minutes of silent writing. H4.1 has a brief survey for ONE specific Tell from the past three weeks. The most honest one, not the most polished. If you had multiple, choose the one that revealed the most about your witness practice — the most awkward, the most surprising, the most costly, the most unfinished.”*

*“If you did not do a Tell across the past three weeks, write that. Honest data. ‘Wk 1: forgot. Wk 2: tried but couldn’t find the moment. Wk 3: avoided.’ The cohort holds it.”*

*“Seven minutes. Begin.”*

## During the silent writing

**Cohort Companions float; visible; available if needed. Brief gentle approach if a member visibly struggles.**

**Co-Companion holds time. At 5 min: gentle reminder. At 6 min 30 sec: “Thirty seconds.”**

**Block 4 — Cohort Survey Around the Circle (7:30–8:08, 38 min)**
## Lead Companion script (90 sec)

*“Around the circle. Each of us, briefly — about 90 seconds.”*

*“Four pieces from your worksheet. ONE — receiver type (initials only; relationship category: colleague / classmate / family / neighbour / friend / service provider / other). TWO — context briefly: where, what was happening. THREE — what mode (relational presence / responsive defense / intentional initiative). FOUR — what surfaced — in you, in the receiver, in the moment.”*

*“If you had no Tell, name that briefly: ‘no Tell across the three weeks because \_\_\_\_\_.’ The cohort receives without commentary.”*

*“I will capture key elements on the flip chart as we go. Receiver types, contexts, modes, what surfaces. Not your name; the data.”*

*“I will go first. One Cohort Companion will go second.”*

## How the block runs

**Lead Companion goes first; models the architecture in 90 seconds.**

**One Cohort Companion goes second.**

**Around the circle. Each member 60–90 seconds. With 24 members, that’s about 30 minutes.**

**Lead Companion captures key data on the flip chart in three columns: RECEIVER TYPE / CONTEXT / WHAT SURFACED. Plus a tally of modes. Brief tick marks.**

**If a member starts to interpret rather than report: ‘Hold the interpretation; we will weigh together when the data is in.’**

**If a member’s contribution is exposing of someone in another cohort (cross-cohort family Tell): use initials only; do not let identifying detail enter the room.**

**If a member’s contribution surfaces a Tell that crossed into harm or active receiver crisis: receive briefly without making it the cohort’s focus; pastoral 1:1 within 24 hours.**

**If multiple members had no Tell: capture honestly in the flip chart’s margin (‘6 members no Tell’). Honest data.**

**Block 5 — Pattern Naming + Cohort Weighing (8:08–8:16, 8 min)**
## Lead Companion script

*“What the flip chart shows.”*

*(Lead Companion stands at the flip chart; reads the three columns; names patterns. Specific. Drawn from what was actually contributed.)*

*“Receiver types: [tally]. Most common context: [naming]. Mode distribution: [naming]. The pattern across what surfaced: [naming].”*

*(Examples: ‘Most Tells were in relational-presence mode — 12 of 20. Responsive defense was 5 of 20. Intentional initiative was 3 of 20. Most receivers were colleagues or classmates. Most contexts were ordinary work or school. What surfaced most often was awkwardness, then surprise at the receiver’s openness.’)*

*“What strikes me — \_\_\_\_\_.”*

*(One observation; specific. Examples: ‘Our cohort’s witness is heavy on relational presence; we may be underusing intentional initiative.’ ‘Several Tells produced unexpected openness in the receiver, which suggests the receivers are more ready than the cohort feared.’)*

*“Two or three voluntary contributions: does what I just named ring true to you about us?”*

*(Voluntary; brief; receive without commentary.)*

*“We hold the picture. Wks 5 and 6 differentiate by domain.”*

**Block 6 — Wks 5–6 Differentiation (8:16–8:21, 5 min)**
## Lead Companion script

*“Wks 5 and 6 differentiate the daily witness by domain. H4.3 has the reference card.”*

*“WK 5 — HOUSEHOLD WITNESS. Family-of-origin, present household, immediate domestic relationships. The most consequential close-to-home witness for many of us; also often the costliest. Standing pairs do harder pair work that week.”*

*“WK 6 — VOCATIONAL WITNESS. Workplace, school, the daily-rhythm context where most of us spend the bulk of our hours. The most weekly-recurrent witness; also where the three modes are most visibly tested.”*

*“Wk 7 begins mission far. The body sent into wider contexts beyond the immediate.”*

*“This week’s Tell is in the household domain. Specifically: in your household or family-of-origin, where might you already be a witness, and where might you be avoiding being one? The Tell can be small — a specific honest exchange, a specific named-and-addressed dynamic, a specific witnessed prayer over a meal. Bring the data to Wk 5.”*

**Block 7 — Feedback Round and Closing Container (8:21–8:30, 9 min)**
## Script


*“Before we close the container: the round. I led tonight, so I go first — what I think went well, and what I would do differently. [Leader answers, specific and brief.] Now the room, same two questions about the evening. Went well; do differently. About the evening and how we ran it — never about what any person brought, said, or heard.”*

*(Receive without defending. Then the teach-back, leader only: “If I were to teach tonight’s process to someone, here is what I would tell them.” One or two sentences. Then close the container as written below.)*
*“One word from each of you, around the circle. How does this evening’s data look land in you?”*

*(Around the circle. 5 seconds per person.)*

*“Good. Wk 5 is household witness. Bring the household-domain Tell from this week. If anything tonight surfaced material heavier than the week can hold, standing pair first, Cohort Companion next.”*

*“Aaronic blessing. Hands up if you want.”*

*“The Lord bless you and keep you; the Lord make His face shine on you and be gracious to you; the Lord turn His face toward you and give you peace.”*

*“See you Tuesday.”*

**Differentiation by Cohort**

*Tonight there is no cohort split. Cohort Companions read their own cohort’s members across the session.*

**Junior teens (12–14)**

## Adjustments

**Junior teens often have Tells that look small to the parent cohort and feel large to the junior teen. The Lead Companion’s frame at the start (scale of context is not scale of faithfulness) is for the junior teens specifically.**

**Watch for: junior teens whose Tell-receiver is a peer in distress (school crisis, family rupture, mental-health concern). Pastoral 1:1 immediately; mandatory-reporting where applicable.**

**Watch for: junior teens whose Tell-mode was almost entirely relational presence (which is appropriate for the developmental stage; do not push toward intentional initiative prematurely).**

**Watch for: junior teens whose Tell did not happen because they had no language for what to say. Receive cleanly; H4.2’s three-mode card opens new vocabulary.**

**Senior teens (15–18)**

## Adjustments

**Senior teens often span all three modes already; the data review may surface that one mode dominates and the others are underdeveloped. Honour without pressing.**

**Watch for: the senior whose Tell-receiver was a fellow senior in active crisis (college decision distress, relational rupture, mental-health concern). Pastoral 1:1 within the week.**

**Watch for: the senior whose Tell crossed into theological depth that they had not been prepared for (a friend asked the resurrection question; the senior froze; the moment passed). Pastoral 1:1 to walk what to do with the next opening.**

**Watch for: the senior whose Tell was performative — reported to land well in the cohort. Cohort Companion notices privately.**

**Parents**

## Adjustments

**Parents often have Tells across multiple domains in any given week (household + vocational + neighbourhood). The discipline tonight is choosing ONE for the survey.**

**Watch for: the parent whose Tell happened with their adult child outside the cohort. Honour; this is often where parents’ most consequential household witness happens.**

**Watch for: the parent whose Tell happened with their teen IN the cohort. Initials only; no identifying detail; cross-cohort awareness held privately.**

**Watch for: the parent whose Tell-pattern is heavy on responsive defense (responding to questions; rarely initiating). Receive without judgment; Wk 6 will surface vocational opportunity for intentional initiative.**

**Watch for: the parent whose Tell crossed into long-running marital tension or family-of-origin material that surfaces tonight publicly. Brief redirect; pastoral 1:1.**

**Closing Practice in Detail**

Standard pattern. The flip-chart data and the Lead Companion’s pattern naming are the closing’s deeper work; the formal closing is brief.

**Between-Session Practice**

## This week’s practice

**Continue the weekly Tell, with this week specifically directed to the HOUSEHOLD domain. Family-of-origin, present household, immediate domestic relationships.**

**Daily noticing question shifts: ‘Where in my house am I a witness already, and where am I avoiding being one?’**

**Standing-pair check-in this week: ‘What is the household witness asking of you specifically?’**

**All ongoing practices continue. Rhythm Card, daily PROAPT, weekday Tells within the cohort, Examen, named-knot sit, shadow noticing.**

**Companion Team Debrief Prompts**

The team meets the day after Wk 4 for a 60-minute debrief.

**Signs the session worked**

**•** The flip chart filled honestly with actual data; the survey produced visible patterns.

**•** The Lead’s pattern naming drew on actual contributions, not invented or imposed.

**•** No comparison shame visibly took root; the frame held across all three cohorts.

**•** Members who had no Tell were received cleanly; honest absence was registered.

**•** By Friday, members report the household-domain Tell is producing specific noticings.

**Signs the session did not work as well**

**•** Members manufactured Tells that did not happen; the data was polluted by performance.

**•** Comparison shame visibly took root; some members shut down across the survey.

**•** The flip chart became the Lead Companion’s interpretive narrative rather than honest data.

**•** A teen’s Tell-receiver was in active crisis and the team’s response was inadequate.

**•** Cross-cohort family material surfaced inadequately handled.

**People to follow up with this week**

**•** Anyone whose Tell-receiver is in active crisis.

**•** Anyone whose Tell-pattern revealed burnout in the practice.

**•** Anyone who has had no Tell three weeks running — not for shame; for noticing what may be in the way.

**•** Any cross-cohort family material that surfaced; pastoral conversation with both the teen and the parent separately.

**•** Any senior whose Tell crossed into theological depth they were not prepared for.

**Handouts**

Four handouts for Wk 4.

**•** H4.1 — Tell Survey Worksheet

**•** H4.2 — Three-Mode Witness Reference

**•** H4.3 — Domain Differentiation Card (household / vocational / third-place)

**•** H4.4 — Between-Session Practice Card (Wk 4)

**Handout H4.1 — Tell Survey Worksheet**

*Used during Block 3 silent writing. ONE specific Tell from the past three weeks. The most honest one.*

**1. The receiver**

Initials only. Relationship category.

Initials: \_\_\_\_\_\_\_\_\_\_\_ Category (colleague / classmate / family / neighbour / friend / service provider / other): \_\_\_\_\_\_\_\_\_\_\_

**2. The context**

Where, what was happening. One or two sentences.

\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**3. The mode**

Circle the dominant mode of this Tell.

## RELATIONAL PRESENCE — the witness was who you were WITH them.

**RESPONSIVE DEFENSE — they asked; you answered.**

**INTENTIONAL INITIATIVE — you initiated specifically.**

**4. What surfaced**

In you, in the receiver, in the moment.

In you: \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

In them: \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

In the moment: \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

**If you did not do a Tell across the three weeks**

Wk 1 of the three: \_\_\_\_\_\_\_\_\_\_\_ What got in the way: \_\_\_\_\_\_\_\_\_\_\_

Wk 2 of the three: \_\_\_\_\_\_\_\_\_\_\_ What got in the way: \_\_\_\_\_\_\_\_\_\_\_

Wk 3 of the three: \_\_\_\_\_\_\_\_\_\_\_ What got in the way: \_\_\_\_\_\_\_\_\_\_\_

**Handout H4.2 — Three-Mode Witness Reference**

*Three modes of witness from the New Testament. All three count; all three have characteristic risks.*

**Mode 1 — Relational Presence**

## ANCHOR. John 13:35 — ‘By this all people will know you are my disciples, if you have love for one another.’

**WHAT IT IS. The body’s love made visible. The witness is who you ARE with people — your integrity in ordinary life, your presence under suffering, your faithfulness in small things.**

**WHEN IT FITS. Most ordinary days. Most ordinary contexts. The 80% of witness that does not require words.**

**RISK. Withholding. The witness becomes invisible because nothing is ever said. Words eventually need to be added; relational presence alone is incomplete.**

**Mode 2 — Responsive Defense**

## ANCHOR. 1 Peter 3:15 — ‘always being prepared to make a defense to anyone who asks you for a reason for the hope that is in you; yet do it with gentleness and respect.’

**WHAT IT IS. Someone asks; you answer. The witness is reactive but specific — the gentleness and respect matter; the answer matters; the timing is set by the receiver.**

**WHEN IT FITS. When the receiver initiates. When the question opens. When the moment arrives unprompted.**

**RISK. Reactivity. The witness only happens when triggered. The body that is only ever responsive may be hiding behind passivity.**

**Mode 3 — Intentional Initiative**

## ANCHOR. Colossians 4:5–6 — ‘Walk in wisdom toward outsiders, making the best use of the time. Let your speech always be gracious, seasoned with salt, so that you may know how you ought to answer each person.’

**WHAT IT IS. The body walks toward outsiders. Specifically. Gracefully. The witness is proactive; the body moves toward the world with seasoned speech.**

**WHEN IT FITS. When the Spirit moves you toward someone. When the time is right and the relationship is ready. When holding back would be unfaithful.**

**RISK. Pressing. The witness becomes coercive when the body forces its initiative beyond what the moment will hold. Salt seasons; it does not preserve by force.**

**Pattern questions for your own practice**

Which mode dominates your weekly Tells? Which mode is hardest for you? Which mode does the Spirit seem to be inviting more of in this season? Bring these questions to your standing pair.

**Handout H4.3 — Domain Differentiation Card**

*Three domains of close-to-home witness. Wk 5 engages household; Wk 6 engages vocational; Wk 7 begins mission far.*

**HOUSEHOLD (Wk 5)**

## WHO. Family-of-origin (parents, siblings, extended family). Present household (spouse, children, those you live with). Immediate domestic relationships.

**WHY HARD. The receivers know the witness’s history. Performance is impossible; the body has been seen for decades. The witness has to be honest about its own formation.**

**WHY CONSEQUENTIAL. The Shema’s formation mechanism happens here — ‘teach them diligently to your children, talk about them when you sit in your house.’ The household is the unit Vol 1’s proposed Household Formation Law engages.**

**VOCATIONAL (Wk 6)**

## WHO. Workplace colleagues (parents). School classmates and teachers (teens). Business partners, professional contacts, regular service relationships.

**WHY RECURRENT. This is where most members spend the bulk of their hours. The witness is repeated, observed, and accumulated over years.**

**WHY VISIBLE. The three modes are most visibly tested here — relational presence under deadlines, responsive defense when colleagues ask hard questions, intentional initiative when the Spirit opens a moment.**

**THIRD-PLACE (Wk 7 onward)**

## WHO. Neighbours, fellow church members outside the cohort, hobby groups, regular service providers, community connections that are not household and not vocational.

**WHY DIFFERENT. Lower stakes than household; lower frequency than vocational; the witness is often relationally specific without being deep.**

**WHY MISSIONALLY IMPORTANT. The third-place network is often where mission far emerges — a casual contact becomes a significant relationship; a neighbour becomes a friend; a hobby group becomes a context for sustained witness over years.**

**Handout H4.4 — Between-Session Practice (Wk 4)**

## This week’s practice — four pieces

**ONE — This week’s outside-cohort Tell is in the HOUSEHOLD domain. Family-of-origin, present household, immediate domestic relationships. The Tell can be small; small in this domain often counts more.**

**TWO — Daily noticing question shifts: ‘Where in my house am I a witness already, and where am I avoiding being one?’ Brief; honest; once each morning.**

**THREE — Standing-pair check-in this week: ‘What is the household witness asking of you specifically?’ Bring the answer to next Tuesday’s Wk 5.**

**FOUR — All ongoing practices continue. Rhythm Card, daily PROAPT, weekday Tells within the cohort, Examen, named-knot sit, shadow noticing.**

**If the household domain feels especially heavy**

It is for many people. Some households are estranged; some are grieving; some are in active rupture; some are profoundly steady but contain witness opportunities long avoided. Pastoral 1:1 with your Cohort Companion or Lead Companion this week if needed.

**If you live alone**

The household domain still applies — family-of-origin (parents, siblings, extended family) counts; the people you spend most time with at home (roommates, regular guests) count; the immediate relational world that shapes your daily life is the household. Bring what is honest.

*From John 13:35 — ‘By this all people will know you are my disciples, if you have love for one another.’*

*The body’s love made visible — first to the people closest to us, then to the wider world. The household is where the witness is most exposed and most needed.*
